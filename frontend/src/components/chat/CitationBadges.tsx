...
const SourceIcon: React.FC<{ type: 'knowledge_base' | 'web'; label: string }> = ({ type, label }) => {
  ...
  if (type === 'knowledge_base') {
    return <Database className="h-3.5 w-3.5 3xl:h-4 3xl:w-4 4xl:h-5 4xl:w-5 text-brand-700" />;
  }
  ...
  switch (iconKey) {
    case 'wikipedia':
      return (
        <svg className="h-3.5 w-3.5 3xl:h-4 3xl:w-4 4xl:h-5 4xl:w-5 text-text-muted" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    case 'reddit':
      return (
        <svg className="h-3.5 w-3.5 3xl:h-4 3xl:w-4 4xl:h-5 4xl:w-5 text-semantic-warning-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    case 'medium':
      return (
        <svg className="h-3.5 w-3.5 3xl:h-4 3xl:w-4 4xl:h-5 4xl:w-5 text-text-primary" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <circle cx="12" cy="12" r="10" fill="currentColor" />
          <circle cx="12" cy="12" r="2" fill="white" />
        </svg>
      );
    default:
      return <Globe className="h-3.5 w-3.5 3xl:h-4 3xl:w-4 4xl:h-5 4xl:w-5 text-text-muted" />;
  }
};
...
export const CitationBadges: React.FC<CitationBadgesProps> = ({ sources, messageId }) => {
  ...
  return (
    <div id={`citation-badges-section-${messageId}`} className="mt-3 3xl:mt-4 4xl:mt-5 w-full">
      <div className="flex flex-wrap items-center gap-2 3xl:gap-2.5 4xl:gap-3">
        <span className="font-inter font-normal text-xs 3xl:text-sm 4xl:text-base text-text-secondary mr-1 whitespace-nowrap shrink-0">
          Sources:
        </span>
...
const CitationBadge: React.FC<{ number: number; source: CitationSource }> = ({ number, source }) => {
  ...
  const badgeContent = (
    <>
      ...
      <span className={`
        flex items-center justify-center w-4 h-4 3xl:w-5 3xl:h-5 4xl:w-6 4xl:h-6 rounded-full text-2xs 3xl:text-xs 4xl:text-sm font-medium
  ...
  const baseBadgeClass = "inline-flex items-center gap-1.5 3xl:gap-2 px-2.5 3xl:px-3 4xl:px-3.5 py-1 3xl:py-1.5 4xl:py-2 rounded-full border border-border-ui bg-surface-card hover:bg-surface-subtle hover:border-border-ui transition-colors max-w-full min-w-0";
