      ...
      <div className="container mx-auto max-w-7xl 3xl:max-w-[1600px] 4xl:max-w-[1800px] px-4 md:px-10 3xl:px-12 py-3 3xl:py-4 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0 overflow-x-auto">
          {selected.map((p) => (
            ...
            >
              <span className="font-inter text-xs 3xl:text-[13px] 4xl:text-sm font-medium text-text-primary truncate">
                {p.name}
              </span>
              <button
                type="button"
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            type="button"
            onClick={onClear}
            className="font-inter text-xs 3xl:text-[13px] 4xl:text-sm text-text-muted hover:text-text-primary transition-colors px-2 py-1.5"
          >
            Clear
          </button>
          <button
