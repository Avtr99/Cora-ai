  ...
  >
    {errorMessage ? (
      <div className="flex flex-col items-center gap-2 text-center px-4">
        <span className="font-inter text-xs 3xl:text-sm 4xl:text-base text-text-muted">{errorMessage}</span>
      </div>
    ) : (
      <div className="flex items-center gap-2">
        <span className="w-1.5 h-1.5 rounded-full bg-brand-500 animate-pulse" aria-hidden="true" />
        <span className="font-inter text-xs 3xl:text-sm 4xl:text-base text-text-muted">Loading world map…</span>
